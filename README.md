...
- Resina muestra las columnas Producto, Genética, Lote y Disponible; la cantidad queda expresada siempre en gramos.
- Producto se elige entre Rosin y Resina BHO.
- Al agregar o editar una resina se pueden completar y modificar producto, genética, lote y disponible.
- Requiere ejecutar `Rainbows_V3.24.7_detalle_stock_resina.sql` en Supabase después de la migración V3.24.6.
...
- Si Schwazzing queda pendiente, se muestra durante los días restantes de Flora S4 y deja de arrastrarse al iniciar Flora S5. Los registros de días anteriores y las tareas reprogramadas manualmente permanecen disponibles.
- Evita que tareas automáticas de ciclos anteriores aparezcan en Hoy para las tres salas. No requiere SQL.
...
- La columna Movimiento muestra «Dispensario → Laboratorio»; Cambio muestra los gramos tanto antes como después de confirmar la recepción.
...
## V3.18.1 — Eliminar comandas de Medrano

- Agrega la acción `Eliminar` junto a `Editar` en las comandas del día y en el historial.
...
## V3.18.4 — Editar comandas dispensadas

- Mantiene disponible la acción `Editar` después de marcar una comanda como dispensada.
- Permite corregir producto, cantidad, paciente o fecha también desde el historial.
- Las comandas eliminadas permanecen inalterables.


## V3.18.5 — Menú y reapertura de comandas

- Reemplaza las acciones visibles por un menú de tres puntos con `Editar` y `Eliminar`.
- El menú está disponible tanto en comandas pendientes como dispensadas y en el historial.
- Editar una comanda dispensada la destilda y la devuelve automáticamente a pendientes.
- Mantiene inalterable el registro de las comandas eliminadas.
- Requiere aplicar `Rainbows_V3.18.5_editar_comandas.sql` después de V3.18.3.


## V3.18.6 — Categorías de Laboratorio

- Organiza el stock de Laboratorio en Resina, Aceites, Cremas y Cápsulas.
- Cada categoría abre una pantalla independiente preparada para sus productos y movimientos.
- No requiere migración de base de datos.


## V3.18.7 — Flores y Mostrador

- Divide el stock del Dispensario en `Flores` y `Mostrador`.
- `Flores` conserva sin cambios el stock existente por sala, lote y genética.
- `Mostrador` permite agregar productos, actualizar cantidades y sacar productos de la lista sin borrar sus datos.
- Carga inicialmente Papelitos, Armadores y Picadores.
- Requiere aplicar `Rainbows_V3.18.7_stock_mostrador.sql`.
