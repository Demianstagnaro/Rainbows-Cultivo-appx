...
function loadCalendar(){
  ...
  const prefix=source.slice(0,source.indexOf('function historicalDone('))
  ...
  vm.runInNewContext(`${prefix}\nglobalThis.calendar={rules,parse,ymd,add,diff,cycle,cycleNumber,harvest,harvestCycleNumber,routine,cloneTransfer,vegesOccupied,tasks,setToday:date=>{today=()=>parse(date)}};`,context);
...
test('Schwazzing pendiente deja de arrastrarse al terminar Flora S4',()=>{
  calendar.setToday('2026-10-14');
  const names=date=>calendar.tasks(calendar.parse(date)).map(task=>`${task.room}|${task.task}`);
  for(const flora of ['Flora 1','Flora 2','Flora 3']){
    assert.ok(!names('2026-09-21').includes(`${flora}|Schwazzing`));
  }
  assert.ok(names('2026-10-08').includes('Flora 2|Schwazzing'));
  assert.ok(names('2026-10-13').includes('Flora 2|Schwazzing'));
  assert.ok(!names('2026-10-14').includes('Flora 2|Schwazzing'));
});
