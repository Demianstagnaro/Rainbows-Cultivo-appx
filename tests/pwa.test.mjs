...
test('todos los componentes declaran la versión 3.20.4',()=>{
  assert.match(app,/const APP_VERSION='3\.20\.4'/);
  assert.match(overrides,/RAINBOWS_OVERRIDES_VERSION='3\.20\.4'/);
  assert.match(sw,/const VERSION='3\.20\.4'/);
  assert.equal(manifest.start_url,'./?v=3.20.4');
  for(const asset of ['styles.css','app.js','rainbows-overrides.js','manifest.json']){
    assert.match(html,new RegExp(`${asset.replace('.','\\.')}\\?v=3\\.20\\.4`));
  }
});

test('las mejoras se cargan en la primera visita sin reescribir respuestas',()=>{
  assert.match(html,/<script defer src="rainbows-overrides\.js\?v=3\.20\.4"><\/script>/);
