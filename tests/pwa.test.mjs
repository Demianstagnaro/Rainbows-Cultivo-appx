...
test('todos los componentes declaran la versión 3.19.1',()=>{
  assert.match(app,/const APP_VERSION='3\.19\.1'/);
  assert.match(overrides,/RAINBOWS_OVERRIDES_VERSION='3\.19\.1'/);
  assert.match(sw,/const VERSION='3\.19\.1'/);
  assert.equal(manifest.start_url,'./?v=3.19.1');
  for(const asset of ['styles.css','app.js','rainbows-overrides.js','manifest.json']){
    assert.match(html,new RegExp(`${asset.replace('.','\\.')}\\?v=3\\.19\\.1`));
  }
});

test('las mejoras se cargan en la primera visita sin reescribir respuestas',()=>{
  assert.match(html,/<script defer src="rainbows-overrides\.js\?v=3\.19\.1"><\/script>/);
