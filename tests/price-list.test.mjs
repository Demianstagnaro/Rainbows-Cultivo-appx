...
const sql=read('Rainbows_V3.25.1_catalogo_central_productos.sql');
...
test('las flores se agrupan por tamaño y no por genética',()=>{
  ...
  assert.match(sql,/public\.medrano_precios_flores/i);
});

test('la lista reúne productos de Laboratorio y Mostrador',()=>{
  assert.match(app,/const catalog=state\.medranoProductCatalog\.map/);
  assert.match(app,/medrano_catalogo_productos/);
...
test('los precios se guardan de forma protegida y recalculan comandas pendientes',()=>{
  ...
  assert.match(sql,/create policy medrano_catalogo_select/i);
