...
-- El stock histórico de Laboratorio sólo tenía created_at. Incorporamos
-- updated_at porque el catálogo sincroniza precios y ediciones posteriores.
...
order by s.categoria,lower(btrim(s.nombre)),s.created_at desc
...
begin
  ...
  if p_id is null then
    ...
    update public.medrano_laboratorio_stock set nombre=btrim(p_nombre),catalogo_producto_id=coalesce(p_catalogo_producto_id,catalogo_producto_id),genetica_id=p_genetica_id,
      ...
      cantidad=p_cantidad,unidad=p_unidad,activo=p_activo,updated_at=now() where id=p_id and categoria=p_categoria and categoria<>'flores';
