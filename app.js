...
function renderMedranoOrders(medranoNav,bindModuleNav){
  ...
  const rows=(state.medranoOrders||[]).filter(o=>o.fecha===todayKey);
  app.innerHTML=`${medranoNav}
  ...
  <section class="panel stock-detail-panel" data-stock-table-tools>
    ...
    <div class="stock-table-wrap"><table class="stock-table medrano-orders-table">
      ...
      <tbody>${rows.length?rows.map(o=>`<tr>
        ...
        ${canManageMedrano()?`<td><button type="button" class="secondary compact-button" data-edit-medrano-order="${o.id}">Editar</button></td>`:''}
      </tr>`).join(''):`<tr data-empty-row="1"><td colspan="${canManageMedrano()?5:4}">Todavía no hay comandas registradas hoy.</td></tr>`}</tbody>
    </table></div>
  </section>`;
  bindModuleNav();
  bindStockTableTools(app);
...
function renderSettings(){
  ...
  app.innerHTML=`
    ...
    <section class="panel"><h3>Usuarios</h3><div class="user-list">${state.perfiles.map(p=>{const safeId=escapeHtml(p.id),safeName=escapeHtml(p.nombre||'Sin nombre'),safeEmail=escapeHtml(p.email||''),safeDeleteName=escapeHtml(p.nombre||p.email||'este usuario');return`<div class="user-row"><div><strong>${safeName}</strong><div class="user-email">${safeEmail}</div></div><select class="text-input user-role" data-role="${safeId}">${['administrador','cultivo','medrano'].map(r=>`<option value="${r}" ${normalizeRole(p.rol)===r?'selected':''}>${r.charAt(0).toUpperCase()+r.slice(1)}</option>`).join('')}</select><label class="user-active"><input type="checkbox" data-active="${safeId}" ${p.activo?'checked':''}> Activo</label>${p.id!==state.session.user.id?`<button class="danger user-delete" type="button" data-delete-user="${safeId}" data-delete-name="${safeDeleteName}">Eliminar cuenta</button>`:'<span class="self-account">Tu cuenta</span>'}</div>`}).join('')}</div><p><button id="save-users" class="primary">Guardar usuarios</button></p></section>
