...
function canViewOperations(){
  return ['administrador','encargado','empleado','lectura'].includes(state.profile?.rol||'empleado');
}
...
function canEditTasks(){
  return ['administrador','encargado'].includes(state.profile?.rol||'empleado');
}
...
function generalTaskRow(t){
  ...
  return `<div class="task-row general-task-row ${generalDone(t)?'done':''}">
    ...
    ${canEditTasks()?`<button class="task-menu" data-general-menu="${t.id}">⋮</button>`:''}
  </div>`;
}
...
function bindGeneralTasks(){
  ...
  if(add) add.onclick=()=>openGeneralTask(null);
}
  ...
  app.innerHTML=`<div class="today-layout">
    <div class="today-main">
      ...
      <div class="today-room-list">${rules.map(r=>{
        ...
        return`<section class="room-card"><div class="room-head"><div><div class="room-title">${r.name}</div><div class="stage">${roomStatus(r,d)}</div></div><div class="room-head-actions">${taskCounter(pr.done,pr.total)}${canEditTasks()?`<button class="task-menu" data-room-menu="${r.name}" data-room-date="${ymd(d)}" aria-label="Opciones de ${r.name}">⋮</button>`:''}</div></div><div class="progress"><span style="width:${pr.pct}%"></span></div><div class="room-tasks">${rt.length?rt.map(row).join(''):'<div class="empty-room-tasks">Sin tareas programadas</div>'}</div></section>`}).join('')}</div>
    </div>
    <aside class="general-tasks-panel panel">
      <div class="general-tasks-head"><div><h2>Tareas generales</h2><p class="muted">Pendientes extraordinarios de todo el edificio.</p></div>${canEditTasks()?'<button id="add-general-task" class="primary compact-button">+ Agregar</button>':''}</div>
...
if('serviceWorker'in navigator){
  window.addEventListener('load',()=>navigator.serviceWorker.register('./sw.js?v=3.5.0').catch(console.error));
}
