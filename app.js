...
function cut(date){const groups=[{dest:['Flora 1','Flora 3'],start:parse('2026-05-20'),opp:parse('2026-07-01')},{dest:['Flora 2'],start:parse('2026-07-01'),opp:parse('2026-05-20')}],active=[];for(const g of groups){const approx=Math.floor(diff(date,g.start)/77);for(let o=-2;o<=2;o++){const fl=add(g.start,(approx+o)*77),intake=add(fl,-1);let h=add(g.opp,56);while(diff(h,intake)<=0)h=add(h,77);const exit=add(h,2);if(diff(date,intake)>=0&&diff(date,exit)<0)active.push({dest:g.dest,intake,exit})}}if(!active.length)return{active:false,label:'Vacía'};active.sort((a,b)=>b.intake-a.intake);const x=active[0];return{active:true,label:`Día ${diff(date,x.intake)+1}`,dest:x.dest,exit:x.exit}}
...
function renderRooms(){ $('screen-title').textContent='Salas';if(!state.room){app.innerHTML=`<div class="list">${rules.map(r=>`<section class="room-card" data-room="${r.name}"><div class="room-head"><div><div class="room-title">${r.name}</div><div class="stage">${stage(r,today())}</div></div><span class="pill">Ver</span></div></section>`).join('')}</div>`;app.querySelectorAll('[data-room]').forEach(x=>x.onclick=()=>{state.room=x.dataset.room;render()});return}const r=rr(state.room),cro=r.type==='flora';app.innerHTML=`<button id="back-room" class="secondary">← Volver</button><section class="panel"><h2>${r.name}</h2><p class="muted">${stage(r,today())}</p></section>${cro?`<div class="room-tabs"><button data-tab="summary" class="${state.tab==='summary'?'active':''}">Resumen</button><button data-tab="croquis" class="${state.tab==='croquis'?'active':''}">Croquis</button></div>`:''}${state.tab==='croquis'&&cro?renderCroquis(r):`<div class="section-title">Tareas de hoy</div>${tasks(today()).filter(t=>t.room===r.name).map(row).join('')}`}`;$('back-room').onclick=()=>{state.room=null;state.tab='summary';render()};app.querySelectorAll('[data-tab]').forEach(x=>x.onclick=()=>{state.tab=x.dataset.tab;render()});app.querySelectorAll('[data-bed]').forEach(x=>x.onclick=()=>openBed(x.dataset.bed));app.querySelectorAll('[data-plant]').forEach(x=>x.onclick=()=>openPlant(x.dataset.plant));bind(today())}
...
function renderSettings(){ $('screen-title').textContent='Config';const admin=state.profile?.rol==='administrador';app.innerHTML=`<section class="panel"><h3>Empleados compartidos</h3><textarea id="emps" class="text-input" style="min-height:150px">${state.empleados.map(e=>e.nombre).join('\n')}</textarea></section><section class="panel"><h3>Genéticas compartidas</h3><textarea id="gens" class="text-input" style="min-height:180px">${state.geneticas.map(g=>g.nombre).join('\n')}</textarea></section><button id="save-conf" class="primary">Guardar configuración</button><section class="panel"><h3>Usuarios</h3>${admin?`<div class="user-list">${state.perfiles.map(p=>`<div class="user-row"><div><strong>${p.nombre||'Sin nombre'}</strong><div class="user-email">${p.email||''}</div></div><select class="text-input user-role" data-role="${p.id}">${['administrador','encargado','empleado','lectura'].map(r=>`<option value="${r}" ${p.rol===r?'selected':''}>${r}</option>`).join('')}</select><label class="user-active"><input type="checkbox" data-active="${p.id}" ${p.activo?'checked':''}> Activo</label></div>`).join('')}</div><p><button id="save-users" class="primary">Guardar usuarios</button></p>`:'<p class="muted">Solo los administradores pueden modificar roles y accesos.</p>'}</section><section class="panel"><p>${state.session.user.email}</p><p>Rol: ${state.profile?.rol||'empleado'}</p><p>Versión ${APP_VERSION}</p></section>`;$('save-conf').onclick=saveConfig;if(admin)$('save-users').onclick=async()=>{try{for(const p of state.perfiles){const q=await db.rpc('admin_actualizar_perfil',{objetivo_id:p.id,nuevo_rol:document.querySelector(`[data-role="${p.id}"]`).value,nuevo_activo:document.querySelector(`[data-active="${p.id}"]`).checked});if(q.error)throw q.error}await refresh();alert('Usuarios actualizados.')}catch(e){console.error(e);alert(e.message||'No se pudieron actualizar los usuarios.')}}}
function bind(d){app.querySelectorAll('[data-new]').forEach(x=>x.onclick=()=>openTask(null,x.dataset.new));app.querySelectorAll('[data-task-id]').forEach(x=>x.onchange=async()=>{const t=findTask(x.dataset.taskId,d);if(x.checked){x.checked=false;openWorkers(t)}else await undo(t)});app.querySelectorAll('[data-menu]').forEach(x=>x.onclick=()=>menu(findTask(x.dataset.menu,d)))}
function openWorkers(t){state.pending=t;state.selected.clear();$('worker-options').innerHTML=state.empleados.map(e=>`<button type="button" data-emp="${e.id}">${e.nombre}</button>`).join('');$('worker-options').querySelectorAll('button').forEach(b=>b.onclick=()=>{const id=b.dataset.emp;if(state.selected.has(id)){state.selected.delete(id);b.classList.remove('selected')}else{state.selected.add(id);b.classList.add('selected')}});$('worker-dialog').showModal()}$('cancel-worker').onclick=()=>$('worker-dialog').close();$('confirm-worker').onclick=async()=>{if(!state.selected.size){alert('Elegí al menos una persona.');return}$('worker-dialog').close();await complete(state.pending,[...state.selected])}
function openTask(t,date){state.editTask=t;$('task-dialog-title').textContent=t?'Editar tarea':'Nueva tarea';$('task-date').value=t?.date||date;$('task-room').innerHTML=state.salas.map(s=>`<option value="${s.id}">${s.nombre}</option>`).join('');$('task-room').value=t?.db?.sala_id||sr(t?.room)?.id||state.salas[0]?.id;$('task-name').value=t?.task||'';$('task-detail').value=t?.detail||'';$('task-dialog').showModal()}$('cancel-task').onclick=()=>$('task-dialog').close();$('save-task').onclick=async()=>{const p={fecha:$('task-date').value,sala_id:$('task-room').value,nombre:$('task-name').value.trim(),detalle:$('task-detail').value.trim(),tipo:state.editTask?.type||'extraordinaria',estado:'pendiente',creada_por:state.session.user.id,actualizada_por:state.session.user.id};if(!p.fecha||!p.nombre){alert('Completá fecha y tarea.');return}if(state.editTask?.db?.id)await db.from('tareas').update(p).eq('id',state.editTask.db.id);else await db.from('tareas').insert(p);$('task-dialog').close();await refresh()}
function menu(t){const a=prompt('1 = Editar\n2 = Mover\n3 = Eliminar');if(a==='1')openTask(t,t.date);if(a==='2')move(t);if(a==='3')del(t)}async function move(t){const date=prompt('Nueva fecha (AAAA-MM-DD):',t.date);if(!date)return;if(t.custom)await db.from('tareas').update({fecha:date,tipo:'reprogramada',detalle:`Reprogramada desde ${t.date}`}).eq('id',t.db.id);else{const o=await ensure(t);await db.from('tareas').update({estado:'cancelada'}).eq('id',o.id);await db.from('tareas').insert({fecha:date,sala_id:sr(t.room)?.id,nombre:t.task,detalle:`Reprogramada desde ${t.date}`,tipo:'reprogramada',estado:'pendiente',origen_tarea_id:o.id,creada_por:state.session.user.id})}await refresh()}async function del(t){if(!confirm(`¿Eliminar ${t.task}?`))return;if(t.custom)await db.from('tareas').update({estado:'cancelada'}).eq('id',t.db.id);else{const o=await ensure(t);await db.from('tareas').update({estado:'cancelada'}).eq('id',o.id)}await refresh()}
...
if('serviceWorker'in navigator){
  window.addEventListener('load',()=>navigator.serviceWorker.register('./sw.js?v=3.1.1').catch(console.error));
}
