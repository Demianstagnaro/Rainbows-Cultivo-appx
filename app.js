...
function renderRoomTaskGroups(list){
  ...
  const pending=list.filter(task=>!done(task)),completed=list.filter(done);
  return `${pending.length?`<h3 class="task-status-title status-heading-pending">Tareas pendientes</h3>${pending.map(row).join('')}`:''}${completed.length?`<h3 class="task-status-title status-heading-completed">Tareas realizadas</h3>${completed.map(row).join('')}`:''}`;
}
...
function canManageMedrano(){return ['administrador','medrano'].includes(currentRole())}
function openMedranoLotDialog(room){
  if(!canManageMedrano())return;
  $('medrano-lot-room').value=room;
  $('medrano-lot-code').value='';
...
async function saveMedranoLot(){
  if(!canManageMedrano())throw new Error('No tenés permiso para ingresar lotes en Medrano.');
  const sala=$('medrano-lot-room').value;
  const codigo=$('medrano-lot-code').value.trim();
  const geneticaId=$('medrano-lot-genetic').value||null;
...
function medranoDailyHistory(sector,categoria,period="past"){
  ...
  if((!sector||sector==='laboratorio')&&(!categoria||categoria==='flores'))for(const transfer of state.medranoLabTransfers||[]){
    ...
    rows.push({sector:'laboratorio',categoria:'flores',fecha:medranoTransferDate(transfer),created_at:transfer.created_at,producto:transfer.nombre,lote:transfer.codigo_lote,cantidad_anterior:receipt?.cantidad_anterior??0,cantidad_nueva:receipt?.cantidad_nueva??0,unidad:'g',accion:'Dispensario → Laboratorio',cantidad_traslado:Number(transfer.gramos),usuario_nombre:receipt?`${sender} · Recepción: ${receipt.usuario_nombre}`:sender,transferencia:true,recepcion:!!receipt});
  }
...
function openMedranoLabTransfer(){
  ...
  const lots=state.medranoDispensarioLots.filter(l=>Number(l.gramos_actual)>0);
  $('lab-transfer-lot').innerHTML='<option value="">Seleccionar lote</option>'+lots.map(l=>`<option value="${escapeHtml(l.id)}">${escapeHtml(l.codigo_lote)} · ${escapeHtml(medranoLotGeneticName(l))} · ${formatGrams(l.gramos_actual)}</option>`).join('');
  $('lab-transfer-grams').value='';
  $('lab-transfer-dialog').showModal();
}
async function sendMedranoLabTransfer(){
  const id=$('lab-transfer-lot').value,value=$('lab-transfer-grams').value,grams=Number(value);
  const lot=state.medranoDispensarioLots.find(l=>String(l.id)===id);
  if(!lot||!value||!Number.isFinite(grams)||grams<=0||grams>Number(lot.gramos_actual))throw new Error('Seleccioná un lote y una cantidad mayor a cero que no supere el disponible.');
...
async function saveMedranoCounterItem(){
  ...
  const catalogId=$('medrano-counter-product').value;
  const cantidad=Number($('medrano-counter-quantity').value);
  if(!catalogId)throw new Error('Seleccioná un producto de la Lista de precios.');
...
function renderMedranoDispensarioStock(medranoNav,bindModuleNav){
  ...
  if(!state.medranoDispensarioRoom){
    ...
    app.innerHTML=`${medranoNav}<section class="panel stock-page-head"><div><button id="medrano-stock-list-back" class="secondary compact-button" type="button">← Stock Medrano</button><h2>Dispensario</h2><p class="muted">Stock general disponible en Medrano.</p></div>${medranoStockHistoryButton('dispensario','flores')}</section>
    ${pendingHtml}
    ...
    bindMedranoStockHistoryButtons();
  ...
  const roomLots=lots.filter(l=>l.sala===room).sort((a,b)=>String(b.fecha_ingreso||'').localeCompare(String(a.fecha_ingreso||'')));
...
function renderMedranoPatients(medranoNav,bindModuleNav){
  ...
  <section class="panel stock-detail-panel medrano-patients-panel" data-stock-table-tools>
    ...
    <div class="stock-table-wrap medrano-patients-table-wrap"><table class="stock-table medrano-patients-table">
      <thead><tr><th data-sort-type="text">Nº socio</th><th data-sort-type="text">Nombre</th><th data-sort-type="text">Apellido</th><th data-sort-type="number">Saldo Tokens</th><th data-sort-type="text">DNI</th><th data-sort-type="text">Teléfono</th><th data-sort-type="text">Domicilio</th><th data-sort-type="text">Código de vinculación</th><th data-sort-type="date">Fecha de ingreso</th><th data-sort-type="text">Sexo</th><th data-sort-type="text">Referido</th><th data-sort-type="date">Vencimiento Reprocann</th>${canManageMedrano()?'<th>Acción</th>':''}</tr></thead>
      <tbody>${rows.length?rows.map(p=>`<tr><td><strong>${escapeHtml(p.numero_socio||'')}</strong></td><td>${escapeHtml(p.nombre||'')}</td><td>${escapeHtml(p.apellido||'')}</td><td data-sort-value="${Number(p.saldo_tokens)||0}"><strong>${formatTokens(p.saldo_tokens)}</strong></td><td>${escapeHtml(p.dni||'')}</td><td>${escapeHtml(p.telefono||'—')}</td><td>${escapeHtml(p.domicilio||'—')}</td><td>${escapeHtml(p.codigo_vinculacion||'—')}</td><td data-sort-value="${escapeHtml(p.fecha_ingreso||'')}">${medranoPatientDate(p.fecha_ingreso)}</td><td>${escapeHtml(p.sexo||'—')}</td><td>${escapeHtml(p.referido||'—')}</td><td data-sort-value="${escapeHtml(p.fecha_vencimiento_reprocann||'')}">${medranoPatientDate(p.fecha_vencimiento_reprocann)}</td>${canManageMedrano()?`<td><button type="button" class="secondary compact-button" data-edit-medrano-patient="${p.id}">Editar</button></td>`:''}</tr>`).join(''):`<tr data-empty-row="1"><td colspan="${canManageMedrano()?13:12}">Todavía no hay pacientes registrados.</td></tr>`}</tbody>
    </table></div>
  </section>`;
...
const medranoLabOrderTypes=new Set(['resina','aceites','cremas','capsulas']);
function medranoLabOrderView(order,items){
...
function updateMedranoMultiProduct(row){
  ...
  product.innerHTML='<option value="">Seleccionar producto</option>'+medranoCatalog(tipo,state.editMedranoMultiOrder).map(p=>`<option value="${escapeHtml(p.id)}">${escapeHtml(p.name)} · ${p.tokens>0?`${formatTokens(p.tokens)}/${escapeHtml(p.unit)}`:'sin valor'} · libre ${Number(p.qty-medranoReserved(tipo,p.id,state.editMedranoMultiOrder?.id)).toLocaleString('es-AR')} ${escapeHtml(p.unit)}</option>`).join('');
...
function updateMedranoMultiTotal(){
  const total=[...$('medrano-multi-lines').querySelectorAll('.medrano-multi-line')].reduce((sum,row)=>{const p=medranoCatalog(row.querySelector('[data-order-type]').value,state.editMedranoMultiOrder).find(x=>x.id===row.querySelector('[data-order-product]').value);return sum+(Number(row.querySelector('[data-order-quantity]').value)||0)*(p?.tokens||0)},0);
  $('medrano-multi-token-total').textContent=`Total: ${formatTokens(total)} · ${formatMoney(total*1000)}`;
}
function addMedranoMultiLine(item=null){
  const container=$('medrano-multi-lines'),row=document.createElement('div');row.className='medrano-multi-line';
  row.innerHTML=`<label class="field-label">Tipo<select class="text-input" data-order-type>${Object.entries(medranoOrderCategories).map(([key,label])=>`<option value="${key}">${label}</option>`).join('')}</select></label><label class="field-label">Producto o genética<select class="text-input" data-order-product></select></label><label class="field-label">Cantidad<input class="text-input" data-order-quantity type="number" min="0.01" step="0.01" inputmode="decimal"></label><span data-order-unit class="muted"></span><button class="secondary compact-button" data-remove-order-line type="button" aria-label="Quitar producto">Quitar</button>`;
  container.append(row);row.querySelector('[data-order-type]').value=item?.tipo||'flores';
  row.querySelector('[data-order-type]').onchange=()=>{row.querySelector('[data-order-product]').value='';updateMedranoMultiProduct(row)};
  row.querySelector('[data-order-product]').onchange=()=>updateMedranoMultiUnit(row);
  row.querySelector('[data-order-quantity]').oninput=()=>updateMedranoMultiUnit(row);
  row.querySelector('[data-remove-order-line]').onclick=()=>{row.remove();updateMedranoMultiTotal()};
  updateMedranoMultiProduct(row);
  if(item){row.querySelector('[data-order-product]').value=item.origen_id;row.querySelector('[data-order-quantity]').value=item.cantidad;updateMedranoMultiUnit(row)}
}
function openMedranoMultiDialog(order=null){
  if(!canManageMedrano()||!state.medranoMultiReady)return;
  if(!state.medranoCajaReady){alert('Primero aplicá la migración V3.24.0 de Caja y Tokens en Supabase.');return}
  if(order?.pago_estado==='pagada'){alert('Una comanda pagada no se puede editar. Si necesitás cancelarla, eliminála y los Tokens volverán al saldo del paciente.');return}
  state.editMedranoMultiOrder=order;
  $('medrano-multi-title').textContent=order?'Editar comanda':'Nueva comanda';
  $('medrano-multi-patient').innerHTML='<option value="">Seleccionar paciente</option>'+state.medranoPatients.map(p=>`<option value="${escapeHtml(p.id)}">${escapeHtml(`${p.nombre||''} ${p.apellido||''}`.trim())}</option>`).join('');
  $('medrano-multi-patient').value=order?.paciente_id||'';
  $('medrano-multi-date').value=order?.fecha||ymd(today());
  $('medrano-multi-lines').innerHTML='';
  (order?.items?.length?order.items:[null]).forEach(addMedranoMultiLine);
  updateMedranoMultiTotal();
  $('medrano-multi-dialog').showModal();
}
async function saveMedranoMultiOrder(){
  const patient=$('medrano-multi-patient').value;
  if(!patient)throw new Error('Seleccioná un paciente.');
  const rows=[...$('medrano-multi-lines').querySelectorAll('.medrano-multi-line')];
  if(!rows.length)throw new Error('Agregá al menos un producto.');
  const items=rows.map(row=>({tipo:row.querySelector('[data-order-type]').value,origen_id:row.querySelector('[data-order-product]').value,cantidad:Number(row.querySelector('[data-order-quantity]').value)}));
...
function openMedranoOrderDialog(order=null){
  ...
  if(!canManageMedrano())return;
  state.editMedranoOrder=order||null;
  $('medrano-order-dialog-title').textContent=order?'Editar comanda':'Nueva comanda';
  $('medrano-order-product').value=order?.producto||'';
  $('medrano-order-quantity').value=order?.cantidad??'';
  const sel=$('medrano-order-patient');
  sel.innerHTML='<option value="">Seleccionar paciente</option>'+(state.medranoPatients||[]).map(p=>`<option value="${p.id}">${escapeHtml(`${p.nombre||''} ${p.apellido||''}`.trim())}</option>`).join('');
  sel.value=order?.paciente_id||'';
  $('medrano-order-date').value=order?.fecha||ymd(today());
  $('medrano-order-dialog').showModal();
}
async function saveMedranoOrder(){
  if(!canManageMedrano())throw new Error('No tenés permiso para modificar comandas.');
  const patientId=$('medrano-order-patient').value||null;
  const patient=(state.medranoPatients||[]).find(p=>String(p.id)===String(patientId));
  const payload={
    producto:$('medrano-order-product').value.trim(),
    cantidad:Number($('medrano-order-quantity').value),
    paciente_id:patientId,
    nombre_paciente:patient?`${patient.nombre||''} ${patient.apellido||''}`.trim():null,
    fecha:$('medrano-order-date').value||ymd(today()),
    updated_at:new Date().toISOString()
  };
  if(!payload.producto)throw new Error('Ingresá el producto.');
  if(!Number.isFinite(payload.cantidad)||payload.cantidad<=0)throw new Error('Ingresá una cantidad válida.');
  if(!payload.paciente_id)throw new Error('Seleccioná un paciente.');
  let q;
  if(state.editMedranoOrder?.id){
    q=await db.rpc('editar_comanda_medrano',{
      objetivo_id:state.editMedranoOrder.id,
      producto_nuevo:payload.producto,
      cantidad_nueva:payload.cantidad,
      paciente_id_nuevo:payload.paciente_id,
      nombre_paciente_nuevo:payload.nombre_paciente,
      fecha_nueva:payload.fecha
    });
  }else{payload.creado_por=state.session.user.id;q=await db.from('medrano_comandas').insert(payload);}
  if(q.error)throw q.error;
...
function openMedranoOrderDeleteDialog(order){
  ...
  $('medrano-order-delete-summary').textContent=`${order.producto||'Comanda'} · ${order.multiple?formatTokens(order.tokens_total):order.cantidad??'—'} · ${order.nombre_paciente||'Sin paciente'} · ${order.fecha?parse(order.fecha).toLocaleDateString('es-AR'):'Sin fecha'}`;
...
async function deleteMedranoOrder(orderId,reason=''){
  ...
  const order=findEditableMedranoOrder(orderId);
  if(!order)throw new Error('No se encontró la comanda.');
  const q=order.multiple?await db.rpc('eliminar_comanda_tokens',{p_id:order.id,p_motivo:reason.trim()||null}):await db.rpc('eliminar_comanda_medrano',{objetivo_id:order.id,motivo:reason.trim()||null});
  if(q.error)throw q.error;
...
async function markMedranoOrderDispensed(orderId){
  if(!canManageMedrano())throw new Error('No tenés permiso para confirmar comandas.');
...
function medranoOrderOptionsHtml(order){
  ...
  const editable=!order.multiple||order.pago_estado!=='pagada';
...
function findEditableMedranoOrder(orderId){
  return [...(state.medranoOrders||[]),...(state.medranoDispensedOrders||[])].find(item=>String(item.id)===String(orderId));
}
...
function bindMedranoOrderActions(){
  ...
  app.querySelectorAll('[data-delete-medrano-order]').forEach(button=>button.onclick=()=>{
    const order=findEditableMedranoOrder(button.dataset.deleteMedranoOrder);
    const menu=button.closest('details');if(menu)menu.open=false;
    if(!order)return;
  ...
  app.querySelectorAll('[data-dispense-medrano-order]').forEach(input=>input.onchange=async()=>{
    if(!input.checked)return;
    const order=(state.medranoOrders||[]).find(item=>String(item.id)===String(input.dataset.dispenseMedranoOrder));
    if(!order){input.checked=false;return}
    if(!confirm(`¿Confirmar que la comanda de ${order.producto||'este producto'} fue dispensada?`)){input.checked=false;return}
    input.disabled=true;
    try{await markMedranoOrderDispensed(order.id)}catch(error){console.error(error);input.checked=false;input.disabled=false;alert(error.message||'No se pudo confirmar la comanda.')}
  });
  app.querySelectorAll('.medrano-order-menu').forEach(menu=>menu.ontoggle=()=>{
    if(!menu.open)return;
    app.querySelectorAll('.medrano-order-menu[open]').forEach(other=>{if(other!==menu)other.open=false});
  });
}

function medranoOrderHistoryDate(order){return order.dispensada_fecha||order.fecha||''}
function medranoTransferDate(transfer){return transfer.created_at?new Date(transfer.created_at).toLocaleDateString('en-CA',{timeZone:'America/Argentina/Buenos_Aires'}):''}
...
function renderMedranoOrderHistory(medranoNav,bindModuleNav){
  ...
  if(!y){
    const years=[...new Set(rows.map(o=>medranoOrderHistoryDate(o).slice(0,4)).filter(Boolean))].sort((a,b)=>Number(b)-Number(a));
    app.innerHTML=`${medranoNav}
    ...
    <section class="panel medrano-stock-home"><div class="medrano-section-head"><div><h3>Elegí un año</h3></div></div>
      <div class="medrano-stock-grid">${years.length?years.map(year=>`<button class="medrano-stock-card" type="button" data-order-history-year="${year}"><strong>${year}</strong><span>${rows.filter(o=>medranoOrderHistoryDate(o).startsWith(year+'-')).length} comandas</span></button>`).join(''):'<div class="medrano-empty-stock"><strong>Sin historial</strong><p class="muted">Todavía no hay comandas dispensadas.</p></div>'}</div>
    </section>`;
    bindModuleNav();
  ...
  if(!m){
    const months=[...new Set(rows.filter(o=>medranoOrderHistoryDate(o).startsWith(y+'-')).map(o=>medranoOrderHistoryDate(o).slice(5,7)))].sort((a,b)=>Number(b)-Number(a));
    app.innerHTML=`${medranoNav}
    ...
    <section class="panel medrano-stock-home"><div class="medrano-section-head"><div><h3>Elegí un mes</h3></div></div>
      <div class="medrano-stock-grid">${months.map(month=>`<button class="medrano-stock-card" type="button" data-order-history-month="${month}"><strong>${medranoOrderMonthName(month)}</strong><span>${rows.filter(o=>medranoOrderHistoryDate(o).startsWith(`${y}-${month}-`)).length} comandas</span></button>`).join('')}</div>
    </section>`;
    bindModuleNav();
  ...
  const prefix=`${y}-${m}-`;
  const monthRows=rows.filter(o=>medranoOrderHistoryDate(o).startsWith(prefix));
  const days=[...new Set(monthRows.map(o=>medranoOrderHistoryDate(o).slice(8,10)))].sort((a,b)=>Number(b)-Number(a));
  app.innerHTML=`${medranoNav}
  ...
  <section class="panel medrano-stock-home"><div class="medrano-section-head"><div><h3>Elegí un día</h3></div></div>
    <div class="medrano-stock-grid">${days.map(day=>`<button class="medrano-stock-card" type="button" data-order-history-day="${day}"><strong>${Number(day)} de ${medranoOrderMonthName(m).toLowerCase()}</strong><span>${monthRows.filter(o=>medranoOrderHistoryDate(o).slice(8,10)===day).length} comandas</span></button>`).join('')}</div>
  </section>`;
  bindModuleNav();
...
function renderMedranoOrderHistoryDay(medranoNav,bindModuleNav,dateKey){
  $('screen-title').textContent='Historial de comandas';
  const rows=(state.medranoDispensedOrders||[]).filter(o=>medranoOrderHistoryDate(o)===dateKey);
  app.innerHTML=`${medranoNav}
...
function renderMedranoOrders(medranoNav,bindModuleNav){
  ...
  const todayKey=ymd(today());
  const rows=(state.medranoOrders||[]).filter(order=>!order.dispensada_at);
  const completedToday=(state.medranoOrders||[]).filter(order=>Boolean(order.dispensada_at));
  app.innerHTML=`${medranoNav}
  ...
  ${medranoLoadIssueHtml(['medrano_comandas_multiproducto','medrano_comandas_multiproducto_items'])}
  <section class="panel stock-detail-panel" data-stock-table-tools>
    <div class="stock-section-head"><div><h3 class="status-heading-pending">Comandas pendientes</h3><p class="muted">${rows.length} pendiente${rows.length===1?'':'s'}</p></div>${canManageMedrano()?'<button id="medrano-add-order" class="primary compact-button" type="button">+ Nueva comanda</button>':''}</div>
    ${stockTableToolbar('Buscar por producto, cantidad, paciente o fecha...')}
...
function dispensaryEvents(dateKey){
  const orders=[...(state.medranoOrders||[]),...(state.medranoDispensedOrders||[])]
    .filter(order=>Boolean(order.dispensada_at)||order.requiere_cierre===false)
    .filter(order=>medranoOrderHistoryDate(order)===dateKey)
    .map(order=>({kind:'Comanda dispensada',product:order.producto,quantity:String(order.cantidad??''),detail:order.nombre_paciente||'Sin paciente',actor:order.dispensada_por_nombre||'—',timestamp:order.dispensada_at||`${dateKey}T00:00:00`,order}));
  const movements=(state.medranoLabDispMovements||[]).filter(row=>row.fecha===dateKey)
    .map(row=>({kind:'Laboratorio → Dispensario',product:row.producto,quantity:`${Number(row.cantidad).toLocaleString('es-AR')} ${row.unidad}`,detail:`${medranoLabCategoryName(row.categoria)}${row.lote?` · ${row.lote}`:''}`,actor:row.usuario_nombre||'—',timestamp:row.created_at}));
  const transfers=(state.medranoLabTransfers||[]).filter(row=>medranoTransferDate(row)===dateKey)
    .map(row=>({kind:'Dispensario → Laboratorio',product:row.nombre,quantity:formatGrams(Number(row.gramos)),detail:`${row.codigo_lote} · ${row.estado==='recibido'?'Recibido por Laboratorio':'En viaje'}`,actor:state.perfiles.find(p=>p.id===row.enviado_por)?.nombre||'—',timestamp:row.created_at}));
...
function medranoCategoryTokenPrice(category){return Number(state.medranoCategoryPrices.find(row=>row.categoria===category)?.tokens_por_unidad)||0}
...
function openMedranoCreditDialog(){
  $('medrano-credit-patient').innerHTML='<option value="">Seleccionar paciente</option>'+state.medranoPatients.map(p=>`<option value="${escapeHtml(p.id)}">${escapeHtml(medranoPatientName(p))} · saldo ${formatTokens(p.saldo_tokens)}</option>`).join('');
  $('medrano-credit-tokens').value='';$('medrano-credit-method').value='efectivo';$('medrano-credit-detail').value='';updateMedranoCreditTotal();$('medrano-credit-dialog').showModal();
}
async function saveMedranoCredit(){
...
async function saveMedranoTokenPrice(button){
  ...
  const q=await db.rpc('guardar_precio_lista_medrano',{p_tipo:button.dataset.tokenType,p_id:button.dataset.tokenId||null,p_tamano:button.dataset.tokenSize||null,p_tokens:tokens});
  if(q.error)throw q.error;
  if(button.dataset.tokenType==='flores'){
    const row=state.medranoFlowerPrices.find(item=>item.tamano===button.dataset.tokenSize);if(row)row.tokens_por_gramo=tokens;else state.medranoFlowerPrices.push({tamano:button.dataset.tokenSize,tokens_por_gramo:tokens});
  }else if(button.dataset.tokenType==='resina'){
    const row=state.medranoCategoryPrices.find(item=>item.categoria==='resina');if(row)row.tokens_por_unidad=tokens;else state.medranoCategoryPrices.push({categoria:'resina',tokens_por_unidad:tokens});
    app.querySelectorAll('[data-save-token-price][data-token-type="resina"]').forEach(resinButton=>{const resinRow=resinButton.closest('tr'),resinInput=resinRow.querySelector('[data-token-price]'),resinMoney=resinRow.querySelector('[data-token-money]');resinInput.value=String(tokens);resinInput.closest('td').dataset.sortValue=String(tokens);resinMoney.textContent=formatMoney(tokens*1000);resinMoney.dataset.sortValue=String(tokens*1000)});
  }else{
    const row=state.medranoProductCatalog.find(item=>String(item.id)===String(button.dataset.tokenId));if(row)row.tokens_por_unidad=tokens;
  }
...
function openMedranoCatalogDialog(){
  ...
  $('medrano-catalog-name').value='';
  $('medrano-catalog-unit').value='ml';
  $('medrano-catalog-tokens').value='0';
  $('medrano-catalog-dialog').showModal();
}
async function saveMedranoCatalogProduct(){
  const category=$('medrano-catalog-category').value,name=$('medrano-catalog-name').value.trim(),unit=$('medrano-catalog-unit').value,tokens=Number($('medrano-catalog-tokens').value);
...
function renderMedranoPriceList(medranoNav,bindModuleNav){
  ...
  if(!state.medranoPriceListReady){app.innerHTML=`${medranoNav}<section class="panel stock-page-head"><div><button id="medrano-prices-back" class="secondary compact-button" type="button">← Administración</button><h2>Lista de precios</h2></div></section><section class="panel error-panel"><strong>Falta actualizar el catálogo de productos.</strong><p>Aplicá la migración V3.25.1 en Supabase y luego tocá Reintentar.</p><button id="medrano-retry-load" class="primary compact-button" type="button">Reintentar</button></section>`;bindModuleNav();$('medrano-prices-back').onclick=()=>{state.medranoView='administracion';render()};bindMedranoLoadRetry();return}
  const prices=medranoTokenPriceRows();
  app.innerHTML=`${medranoNav}<section class="panel stock-page-head"><div><button id="medrano-prices-back" class="secondary compact-button" type="button">← Administración</button><h2>Lista de precios</h2><p class="muted">Catálogo central · 1 Token = $1.000 · Rosin y Resina BHO comparten el precio por gramo.</p></div><button id="medrano-catalog-add" class="primary compact-button" type="button">+ Agregar producto</button></section>
  <section class="panel stock-detail-panel" data-stock-table-tools>${stockTableToolbar('Buscar por categoría, producto o unidad...')}<div class="stock-table-wrap"><table class="stock-table"><thead><tr><th data-sort-type="text">Categoría</th><th data-sort-type="text">Producto</th><th data-sort-type="text">Unidad</th><th data-sort-type="number">Tokens por unidad</th><th data-sort-type="number">Equivalente</th><th>Estado</th><th></th></tr></thead><tbody>${prices.length?prices.map(p=>`<tr><td>${escapeHtml(p.category)}</td><td><strong>${escapeHtml(p.name)}</strong></td><td>${escapeHtml(p.unit)}</td><td data-sort-value="${p.tokens}"><input class="text-input token-price-input" data-token-price type="number" min="0" step="0.01" value="${p.tokens}"></td><td data-token-money data-sort-value="${p.tokens*1000}">${formatMoney(p.tokens*1000)}</td><td>${p.active?'Activo':'Inactivo'}</td><td><button class="secondary compact-button" type="button" data-save-token-price data-token-type="${escapeHtml(p.type)}" data-token-id="${escapeHtml(p.id)}" data-token-size="${escapeHtml(p.size||'')}">Guardar</button></td></tr>`).join(''):'<tr data-empty-row="1"><td colspan="7">No hay productos cargados.</td></tr>'}</tbody></table></div></section>`;
  bindModuleNav();bindStockTableTools(app);$('medrano-prices-back').onclick=()=>{state.medranoView='administracion';render()};$('medrano-catalog-add').onclick=openMedranoCatalogDialog;
  app.querySelectorAll('[data-save-token-price]').forEach(b=>b.onclick=async()=>{b.disabled=true;try{await saveMedranoTokenPrice(b)}catch(e){alert(e.message||'No se pudo guardar el valor.')}finally{b.disabled=false}});
}
...
const medranoLaboratoryCategories=[
  {key:'flores',label:'Flores',description:'Materia prima para elaboraciones'},
...
function renderMedranoLaboratoryStock(medranoNav,bindModuleNav,categoryKey=''){
  ...
  if(!category){
    ...
    app.innerHTML=`${medranoNav}<section class="panel medrano-stock-home medrano-module-panel"><div class="medrano-section-head"><button id="medrano-stock-list-back" class="secondary compact-button" type="button">← Stock Medrano</button><div><h2>Laboratorio</h2><p class="muted">Elegí el stock de productos, materias primas o insumos que querés consultar.</p></div>${medranoStockHistoryButton('laboratorio')}</div><div class="medrano-stock-grid">${medranoLaboratoryCategories.map(item=>`<button class="medrano-stock-card" data-medrano-laboratory-category="${item.key}" type="button"><strong>${item.label}</strong><span>${item.description||`Productos de ${item.label.toLowerCase()}`}</span></button>`).join('')}</div></section>`;
    bindModuleNav();bindMedranoStockHistoryButtons();
    $('medrano-stock-list-back').onclick=()=>{state.medranoView='stock';render()};
...
function labOilProfileWeights(item,equivalent){
  const names=String(item.perfil_cannabinoide||'').split('-').filter(Boolean),raw=String(item.proporcion_cannabinoides||'').split(/[-:]/).map(Number);
  if(!names.length||!fullSpectrumProfiles.includes(item.perfil_cannabinoide))return null;
  const ratios=raw.length===names.length&&raw.every(value=>Number.isFinite(value)&&value>0)?raw:names.map(()=>1),total=ratios.reduce((sum,value)=>sum+value,0);
  return names.map((name,index)=>({name,value:equivalent*ratios[index]/total}));
}
function labOilCombinedProfile(sources){
  const weights=new Map();
  for(const source of sources)for(const part of labOilProfileWeights(source.item,source.equivalent)||[])weights.set(part.name,(weights.get(part.name)||0)+part.value);
  const order=['THC','CBD','CBN','Otro'],names=order.filter(name=>weights.has(name));
  if(!names.length)return null;
  if(names.includes('Otro')&&names.length>1)return{profile:'Otro',ratio:''};
  const minimum=Math.min(...names.map(name=>weights.get(name))),values=names.map(name=>Math.round(weights.get(name)/minimum*100)/100);
  return{profile:names.join('-'),ratio:names.length>1?values.join('-'):''};
}
function labOilBaseDraft(){
  ...
  const sources=[],bases=[],parts=[];let totalVolume=0,resinEquivalent=0;
  for(const {material,item} of resolved){
    if(material.category==='resina'&&item.unidad==='g'&&fullSpectrumProfiles.includes(item.perfil_cannabinoide)){
      totalVolume+=material.cantidad;resinEquivalent+=material.cantidad;sources.push({item,equivalent:material.cantidad});parts.push(`${material.cantidad.toLocaleString('es-AR')} g ${item.nombre}`);
    }else if(material.category==='aceites'&&item.unidad==='ml'&&item.es_aceite_base&&Number(item.concentracion_denominador)>1&&fullSpectrumProfiles.includes(item.perfil_cannabinoide)){
      const equivalent=material.cantidad/Number(item.concentracion_denominador);totalVolume+=material.cantidad;resinEquivalent+=equivalent;sources.push({item,equivalent});if(item.base_aceite)bases.push(item.base_aceite);parts.push(`${material.cantidad.toLocaleString('es-AR')} ml ${item.nombre}`);
    }else if(material.category==='insumos'&&item.unidad==='ml'){
      totalVolume+=material.cantidad;bases.push(item.nombre);parts.push(`${material.cantidad.toLocaleString('es-AR')} ml ${item.nombre}`);
    }else return null;
  }
  const combined=labOilCombinedProfile(sources),concentration=totalVolume/resinEquivalent;
  if(!sources.length||!combined||!Number.isFinite(concentration)||concentration<=1)return null;
...
function openMedranoLabJobDialog(type='aceite_base',job=null){
  ...
  state.editMedranoLabJob=job;
  $('lab-job-title').textContent=job?'Editar trabajo':type==='comanda_paciente'?'Nueva comanda de Laboratorio':'Nuevo trabajo de producción';
  $('lab-job-type').querySelector('option[value="comanda_paciente"]').disabled=!job&&type!=='comanda_paciente';
  $('lab-job-type').value=job?.tipo||type;
  $('lab-job-product').value=job?.producto||'';
  ...
  $('lab-job-dialog').showModal();
}
async function saveMedranoLabJob(){
  if(!canManageMedrano()||!state.medranoLabJobsReady)throw new Error('No está habilitado el registro de trabajos.');
  const tipo=$('lab-job-type').value,producto=$('lab-job-product').value.trim(),paciente=$('lab-job-patient').value.trim();
  const raw=$('lab-job-quantity').value,cantidad=raw===''?null:Number(raw),unidad=raw===''?null:$('lab-job-unit').value;
  const detalle=$('lab-job-detail').value.trim();
  if(!medranoJobTypes[tipo]||!producto||producto.length>180||tipo==='comanda_paciente'&&!paciente||paciente.length>180||detalle.length>2000||cantidad!==null&&(!Number.isFinite(cantidad)||cantidad<=0||!unidad||unidad==='unidades'&&!Number.isInteger(cantidad)))throw new Error('Revisá el tipo, producto, paciente y cantidad.');
  const q=await db.rpc('guardar_trabajo_laboratorio',{p_id:state.editMedranoLabJob?.id||null,p_tipo:tipo,p_producto:producto,p_paciente:tipo==='comanda_paciente'?paciente:null,p_cantidad:cantidad,p_unidad:unidad,p_detalle:detalle});
  if(q.error)throw q.error;
...
function renderMedranoLaboratory(medranoNav,bindModuleNav,history=false){
  ...
  const todayKey=ymd(today()),jobs=state.medranoLabJobs||[];
  const active=jobs.filter(j=>['pendiente','en_proceso'].includes(j.estado));
  const doneToday=jobs.filter(j=>['finalizado','cancelado'].includes(j.estado)&&medranoJobDay(j.finalizado_at||j.updated_at)===todayKey);
  const older=jobs.filter(j=>['finalizado','cancelado'].includes(j.estado)&&medranoJobDay(j.finalizado_at||j.updated_at)<todayKey);
...
function renderMedrano(){
  ...
  if(mv==='administracion'){
    ...
    $('medrano-open-caja').onclick=()=>{state.medranoView='administracion-caja';render()};
    $('medrano-open-prices').onclick=()=>{state.medranoView='administracion-precios';render()};
    return;
  }
  ...
  if(mv==='dispensario'){
    renderMedranoDispensary(medranoNav,bindModuleNav);return;
  }
  if(mv==='dispensario-historial'){renderMedranoDispensaryHistory(medranoNav,bindModuleNav);return}
  if(mv==='laboratorio'){renderMedranoLaboratory(medranoNav,bindModuleNav);return}
  if(mv==='laboratorio-historial'){renderMedranoLaboratory(medranoNav,bindModuleNav,true);return}
  if(mv==='stock-historial'){renderMedranoStockHistory(medranoNav,bindModuleNav);return}
  if(mv==='stock'){
    $('screen-title').textContent='Stock Medrano';
    app.innerHTML=`${medranoNav}<section class="panel medrano-stock-home medrano-module-panel"><div class="medrano-section-head"><div><h2>Stock Medrano</h2><p class="muted">Elegí el sector de stock que querés consultar.</p></div>${medranoStockHistoryButton()}</div><div class="medrano-stock-grid"><button class="medrano-stock-card" data-medrano-stock="dispensario" type="button"><strong>Dispensario</strong><span>Stock de Dispensario</span></button><button class="medrano-stock-card" data-medrano-stock="laboratorio" type="button"><strong>Laboratorio</strong><span>Stock de Laboratorio</span></button></div></section>`;
    bindModuleNav();bindMedranoStockHistoryButtons();
...
function bindMedranoDialogActions(){
  $('medrano-payment-dialog').onclose=()=>{if(state.pendingMedranoPayment){state.pendingMedranoPayment=null;render()}};
  ...
  $('cancel-medrano-catalog').onclick=()=>closeDialog('medrano-catalog-dialog');
  $('medrano-catalog-category').onchange=()=>{$('medrano-catalog-unit').value=$('medrano-catalog-category').value==='aceites'?'ml':$('medrano-catalog-category').value==='mostrador'||$('medrano-catalog-category').value==='capsulas'?'unidades':'g'};
  $('save-medrano-catalog').onclick=async()=>{const b=$('save-medrano-catalog');b.disabled=true;try{await saveMedranoCatalogProduct()}catch(e){console.error(e);alert(e.message||'No se pudo guardar el producto.')}finally{b.disabled=false}};
...
function scheduleDayChangeRefresh(){
  if(state.dayChangeTimer)clearTimeout(state.dayChangeTimer);
  const now=new Date();
  const next=new Date(now.getFullYear(),now.getMonth(),now.getDate()+1,0,0,1);
  state.dayChangeTimer=setTimeout(()=>{state.dayChangeTimer=null;if(state.session)refresh()},next-now);
}
function render(){scheduleDayChangeRefresh();const isPalestina=renderSiteShell();bindMedranoDialogActions();const cb=$('header-config');if(cb&&isPalestina){const ok=currentRole()==='administrador';cb.hidden=!ok;cb.style.display=ok?'inline-flex':'none';cb.onclick=()=>{state.view='settings';state.room=null;state.roomDay=null;state.day=null;render()}}const hb=$('header-help');if(hb&&isPalestina){hb.onclick=()=>{state.view='help';state.room=null;state.roomDay=null;state.day=null;render()}}if(!isPalestina){renderMedrano();return} $('today-label').textContent=nice(today());
...
function stockSearchNormalize(value){
  return String(value??'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().trim();
}
function stockSortValue(cell,type){
  const raw=cell?.dataset?.sortValue ?? cell?.textContent ?? '';
  if(type==='number'){
    const cleaned=String(raw).replace(/[^0-9,.-]/g,'').replace(/\.(?=.*\.)/g,'').replace(',','.');
    const n=Number(cleaned);return Number.isFinite(n)?n:-Infinity;
  }
  if(type==='date'){
    const iso=cell?.dataset?.sortValue;
    if(iso){const t=Date.parse(iso);if(Number.isFinite(t))return t;}
  ...
  return stockSearchNormalize(raw);
}
function bindStockTableTools(root=document){
  root.querySelectorAll('[data-stock-table-tools]').forEach(wrapper=>{
    const input=wrapper.querySelector('[data-stock-search]');
    const table=wrapper.querySelector('table');
    if(!table)return;
    const rows=()=>Array.from(table.tBodies?.[0]?.rows||[]).filter(r=>!r.dataset.emptyRow);
    const applyFilter=()=>{
      const q=stockSearchNormalize(input?.value||'');
      rows().forEach(row=>{row.hidden=!!q&&!stockSearchNormalize(row.textContent).includes(q)});
    };
    if(input)input.addEventListener('input',applyFilter);
    table.querySelectorAll('th[data-sort-type]').forEach((th,index)=>{
      th.classList.add('stock-sortable');
      th.setAttribute('role','button');
      th.tabIndex=0;
      const sort=()=>{
        const type=th.dataset.sortType||'text';
        const col=Number(th.dataset.sortIndex??index);
        const dir=th.dataset.sortDir==='asc'?'desc':'asc';
        table.querySelectorAll('th[data-sort-type]').forEach(h=>{h.dataset.sortDir='';h.classList.remove('sort-asc','sort-desc')});
        th.dataset.sortDir=dir;th.classList.add(dir==='asc'?'sort-asc':'sort-desc');
        const body=table.tBodies[0];
        rows().sort((a,b)=>{
          const av=stockSortValue(a.cells[col],type),bv=stockSortValue(b.cells[col],type);
...
function renderStock(){
  ...
  if(!selected){
    ...
    <section class="panel stock-detail-panel stock-room-cycles" data-stock-table-tools><h3>Ciclos de cosecha</h3>${stockTableToolbar('Buscar por ciclo, stock o cantidad de genéticas...')}<div class="stock-table-wrap"><table class="stock-table stock-cycle-list-table"><thead><tr><th data-sort-type="number">Ciclo</th><th data-sort-type="number">Stock inicial</th><th data-sort-type="number">Stock actual</th><th data-sort-type="number">Genéticas</th><th></th></tr></thead><tbody>${cycles.length?cycles.map(c=>`<tr data-stock-cycle-row="${c.id}"><td data-sort-value="${Number(c.ciclo)||0}"><strong>Ciclo ${c.ciclo}</strong></td><td data-sort-value="${Number(c.stock_inicial)||0}">${formatGrams(Number(c.stock_inicial)||0)}</td><td data-sort-value="${stockCycleCurrent(c)}"><strong>${formatGrams(stockCycleCurrent(c))}</strong></td><td data-sort-value="${stockCycleItems(c.id).length}">${stockCycleItems(c.id).length}</td><td><button type="button" class="secondary compact-button stock-cycle-open" data-stock-cycle="${c.id}">Ver</button></td></tr>`).join(''):'<tr data-empty-row="1"><td colspan="5">No hay ciclos cargados para esta sala.</td></tr>'}</tbody></table></div></section>`;
    bindStockTableTools(app);
