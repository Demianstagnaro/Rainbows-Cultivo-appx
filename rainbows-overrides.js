const RAINBOWS_OVERRIDES_VERSION='3.17.0';
...
function renderAmendments(){const app=document.getElementById('app'),title=document.getElementById('screen-title');if(!app||!title)return;title.textContent='Enmiendas';app.innerHTML='<section class="panel amendment-intro"><h2>Enmienda completa</h2><p class="muted">Composición y dosis por etapa del ciclo.</p></section>'+Object.entries(amendments).map(([name,rows])=>amendmentTable(name,rows)).join('');document.querySelectorAll('.top-nav button').forEach(b=>b.classList.toggle('active',b.dataset.view==='amendments'))}
function isPalestina(){return document.getElementById('site-palestina')?.classList.contains('active')!==false}
function ensureAmendmentsNav(){const nav=document.querySelector('.top-nav');if(!nav)return;let btn=nav.querySelector('[data-view="amendments"]');if(!btn){btn=document.createElement('button');btn.type='button';btn.dataset.view='amendments';btn.textContent='Enmiendas';const stock=nav.querySelector('[data-view="stock"]');stock?nav.insertBefore(btn,stock):nav.appendChild(btn);btn.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();if(isPalestina())renderAmendments()})}btn.hidden=!isPalestina()}
